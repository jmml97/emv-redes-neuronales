...
En la teoría matemática de las redes neuronales artificiales, el teorema de aproximación universal establece que una red feed-forward con una sola capa oculta que contiene un número finito de neuronas puede aproximarse a funciones continuas en subconjuntos compactos de $R_n$, bajo suposiciones leves sobre la función de activación. El teorema establece que las redes neuronales simples pueden representar una amplia variedad de funciones interesantes cuando se les dan parámetros apropiados; sin embargo, no toca la capacidad de aprendizaje algorítmico de esos parámetros.

Aunque las redes feed-forward con una sola capa oculta son aproximadores universales, el ancho de tales redes tiene que ser exponencialmente grande. En 2017 Lu et al. Probaron el Teorema de aproximación universal para redes neuronales profundas con ancho acotado. En particular, demostraron que las redes de ancho $n + 4$ con funciones de activación ReLU pueden aproximarse cualquier función integrable de Lebesgue en el espacio de entrada n-dimensional con respecto a la distancia $L ^ {1}$ si la profundidad de la red puede crecer.

Matemáticamente la expresión del teorema se puede expresar en dos versiones: la aplicada a un ancho no acotado y al ancho acotado.
\textbf{Caso ancho no acotado:}

Sea $\varphi:\mathbb{R} \rightarrow \mathbb{R}$ una función no constante, acotada y continua, llamada la función de activación e $I_m$ el hipercubo de la unidad de dimensión $m$ , $[0,1]^m$. El espacio de funciones continuas de valor real en $I_m$ se denota como $C(I_m)$. Entonces, dado un $\epsilon > 0$ y una función $f \in C(I_m)$, existe un entero $N$, constantes reales $v_i , b_i \in \mathbb{R}$ y vectores reales $w_i \in \mathbb{R}^m$ para $i =1 \cdots N$,  tales que podemos definir:
$$ F (x) = \sum_{{i = 1}}^{{N}} v_{i} \varphi \left (w_ {i} ^ {T} x + b_ {i} \right) $$
Como una realización aproximada de la función $f$, que es:
$$|F(x)-f(x)| < \epsilon \ \ \forall x \in I_m$$
 En otras palabras, las funciones de la forma $F(x)$ son densas en $C(I_m)$.
Esto funciona también , tomando cualquier subconjunto compacto de $\mathbb{R}^m$ , en lugar de $I_m$.

\textbf{Caso ancho acotado:}

Para cualquier función Lebesgue-integrable $f: \mathbb{R}^n \rightarrow \mathbb{R}$ y cualquier $\epsilon >0$, existe una red completamente conectada con activación ReLU A con ancho $d_m \leq n+4$, tal que la función $F_A$ representada por esta red satisface :

$${\displaystyle \int_{\mathbb{R}^{n}}\left|f(x)-F_{A}(x)\right|\mathrm {d} x<\epsilon } $$
...
\item \textbf{Aprendizaje por refuerzo. } El objetivo de este tipo de
  ...
  un coste (long-term) No me queda claro.
\end{itemize}

\subsection{Autoencoders}

Un autoencoder es un tipo de read neuronal de aprendizaje no supervisado que aprende a copiar su entrada en la
salida, tiene una capa interna que constituye el <<código>> con
el que queremos representar la entrada. Se encuentra dividida en dos partes
principales:
\begin{itemize}
  \item \textbf{Codificador. }Conjunto de capas que transforman la entrada al
    <<código>>.
  \item \textbf{Decodificador. }Conjunto de capas que transforman el
    <<código>> a la entrada original
\end{itemize}
...
En el caso mas simple donde solo existe una capa oculta, la fase de codificación
